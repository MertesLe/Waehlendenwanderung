  ...
  data %>%
    dplyr::mutate(
      Gemeindeschlüssel = dplyr::if_else(
        is.na(Land) | is.na(Regierungsbezirk) | is.na(Kreis) | is.na(Gemeinde),
        NA_character_,
        paste0(
          stringr::str_pad(Land, width = 2, pad = "0"),
          stringr::str_pad(Regierungsbezirk, width = 1, pad = "0"),
          stringr::str_pad(Kreis, width = 2, pad = "0"),
          stringr::str_pad(Gemeinde, width = 3, pad = "0")
        )
      )
    )
}
